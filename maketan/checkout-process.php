<?php
    ...
    $sql_cek_stok = "select * from cart inner join tb_produk on cart.id_produk = tb_produk.id_produk where cart.user_id=$id_user";
    ...
    while($data_cek_stok = mysqli_fetch_array($query_cek_stok)){
        if(($data_cek_stok["stok"] - $data_cek_stok["jumlah"]) < 0){
            ...
            $sql_stok = "update tb_produk set stok=$stok_sisa where id_produk=$id_produk";
    ...
    $sql_keranjang = "select * from cart where user_id=$id_user";
